...
<div class="col-sm-9">
<div class="panel panel-default">
<div class="panel-heading">Ask Us</div>
<div class="panel-body">
<?php 
include("../db/connection.php");
$query = "select * from sign_up where email = '$email'";
...
while($row = mysqli_fetch_array($run)){
	...
	$email = $row['email'];
	?>
<form class="form-horizontal" action="ask_us.php" method="post">
...
<div class="col-sm-4">
<input type="text" class="form-control" name="emp_email" value="<?php echo $email; ?>" placeholder="Employee Email" required="required" readonly="readonly">
</div>
</div>
<div class="form-group">
<label for="question" class="col-sm-4 control-label">Question</label>
<div class="col-sm-4">
<textarea rows="6" cols="50" class="form-control" name="question" id="show_num" required></textarea>
</div>
</div>
<button type="submit" name="ask" class="btn btn-default pull-right">Ask</button>
</form>
<?php
}
?>
...
include ("../db/connection.php");
    if(isset($_POST['ask'])){
    	...
    	$query = "INSERT INTO ask_listen (emp_id,email,question) VALUES ('$emp_id','$emp_email','$question')";
        $run = mysqli_query($con,$query);
        if($run){
        	echo "<b>Success! Your Query has been forwarded to Admin</b>";
        }
		else{
