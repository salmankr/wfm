...
<div class="col-sm-9">
<div class="panel panel-default">
<div class="panel-heading">Answer Queries</div>
<div class="panel-body">
<?php 
include("../db/connection.php");
   $query = "SELECT * FROM ask_listen LIMIT 0,1";
   $run = mysqli_query($con,$query);
   $count = mysqli_num_rows($run);
if($count > 0){
while($row = mysqli_fetch_array($run)){
	...
	?>
<form class="form-horizontal" action="we_listen.php" method="post">
...
<div class="col-sm-4">
<input type="text" class="form-control" name="emp_email" value="<?php echo $user_email; ?>" placeholder="Employee Email" required="required" readonly="readonly">
</div>
</div>
<div class="form-group">
<label for="question" class="col-sm-4 control-label">Question</label>
<div class="col-sm-4">
<textarea class="form-control" name="question" placeholder="Question" required readonly="readonly">
<?php echo $question; ?>
</textarea>
</div>
</div>
...
<div class="col-sm-4">
<textarea rows="6" cols="50" class="form-control" name="answer" id="show_num" required="required"></textarea>
</div>
</div>
...
<?php
	...
	echo "<b>No answer query to show.</b>";
}
...
if(isset($_POST['send'])){
	...
	$answer = $_POST['answer'];
    $query = "DELETE FROM ask_listen WHERE question = '$question'";
    $run = mysqli_query($con,$query);
if($run){
    //Mail script
    $to = $emp_email;
	$form = $email;
	$subject = "WorkForce Management System";
	$message = $answer;
	$header = "From: <$form>";
    $mail = mail($to,$subject,$message,$header);
	 if($mail){
    echo "<script> alert('Query has been answered.'); </script>";
    echo "<script> window.open('we_listen.php','_self'); </script>";
	}
}
else{
