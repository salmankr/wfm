...
<div class="panel-body">
<form class="form-horizontal" action="leave_apr.php" method="post">
<?php 
include("../db/connection.php");
       $query = "SELECT * FROM leaves LIMIT 0,1";
       $run = mysqli_query($con,$query);
while($row = mysqli_fetch_array($run)){
...
<div class="col-sm-5">
<textarea rows="10" cols="50" type="date" class="form-control" name="reason" placeholder="Reason" required="required" disabled><?php echo $reason; ?></textarea>
</div>
</div>



<button name="apr_btn" class="btn btn-default pull-right">Done</button>
<?php
}
include("../db/connection.php");
if(isset($_POST['apr_btn'])){
    ...
    $query = "DELETE FROM leaves WHERE frm_dte = '$frm_date' and emp_id = '$emp_id'"; 
    $run = mysqli_query($con,$query);
if($run){
	
	?>
<div class="col-sm-12" style="text-align: center;"><b>Leave Approval/Disapproval Done!</b></div
<?php
}
else{
	echo "not done";
